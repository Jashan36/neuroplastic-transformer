...
nltk.download('punkt')
nltk.download('averaged_perceptron_tagger')  # For factual consistency check
...
epsilon_fact = 0.5  # Factual consistency

# Expanded curriculum with more domains
CURRICULUM = [
...
# Plasticity Wrapper with monitoring enhancements
class NeuroPlasticWrapper(nn.Module):
    def __init__(self, model, gamma, beta, lambda_decay):
        ...
        # Initialize eligibility traces with monitoring
        ...
        with torch.no_grad():
            for name, param in self.model.named_parameters():
                if name in self.eligibility_traces and param.grad is not None:
                    ...
                    # Record trace norm for monitoring
                    trace_norm = torch.norm(self.eligibility_traces[name]).item()
    ...
    def visualize_traces(self, step, output_dir="trace_visualizations"):
        """Visualize eligibility trace evolution"""
        os.makedirs(output_dir, exist_ok=True)
        for name, history in self.trace_history.items():
            if len(history) > 10:  # Only plot if we have enough data
                plt.figure(figsize=(10, 4))
                plt.plot(history)
                plt.title(f"Eligibility Trace Norm: {name}")
...
def calculate_factual_consistency(prompt, response):
    """Check if response introduces new entities not in prompt"""
    prompt_entities = set()
    response_entities = set()
    
    # Extract entities (nouns) from prompt and response
    prompt_pos = nltk.pos_tag(nltk.word_tokenize(prompt))
    response_pos = nltk.pos_tag(nltk.word_tokenize(response))
    
    # Collect nouns (NN, NNS, NNP, NNPS)
    ...
    # Find entities in response not in prompt
    new_entities = response_entities - prompt_entities
    
    # Penalize if there are new entities (they might be hallucinations)
...
for stage in set(s["name"] for s in CURRICULUM):
    domains = next(s["domains"] for s in CURRICULUM if s["name"] == stage)
    datasets[stage] = CurriculumDataset(stage, domains)

# Enhanced training function with context handling
def train_step(batch, curriculum_stage, lambda_reward, global_step):
...
def validate(val_loader, curriculum_stage):
    ...
    with torch.no_grad():
        for instructions, responses, domains in val_loader:
            # Tokenize
            max_length = next(s for s in CURRICULUM if s["name"] == curriculum_stage)["max_length"]
            inputs = tokenizer(
                instructions, 
...
def main_training_loop():
    ...
    for stage in CURRICULUM:
        ...
        train_loader = DataLoader(
            ...
            batch_size=8, 
            shuffle=True,
            collate_fn=lambda batch: list(zip(*batch))  # Unzip into instructions/responses/domains
        )
        ...
        for epoch in range(stage_epochs):
            ...
            for batch_idx, batch in enumerate(train_loader):
                ...
                metrics = train_step(batch, stage_name, decayed_lambda, global_step)
                global_step += 1
            ...
            val_loss = val_metrics["loss_ce"] + decayed_lambda * (
                alpha * val_metrics["efficiency"] +
                beta_sem * val_metrics["semantic"] +
                gamma_div * (1 - val_metrics["diversity"]) +
                delta_ref * val_metrics["reference"] +
                epsilon_fact * val_metrics["factual"]
            )
...
def ood_evaluation(model, tokenizer):
    ...
    for prompt, category in ood_prompts:
        ...
        # Score response
        coherence = random.uniform(0.7, 1.0)  # Placeholder for real metric
        relevance = random.uniform(0.6, 1.0)   # Placeholder
        results.append({
            "category": category,
            "prompt": prompt,
            "response": response,
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load("checkpoints/best_model_dialogues.pt"))
    ...
    # Out-of-distribution evaluation
    ood_results = ood_evaluation(model, tokenizer)
    
    # Initialize inference interface
    print("\n=== Inference Interface ===")
    inference_engine = InferenceInterface(model, tokenizer)
