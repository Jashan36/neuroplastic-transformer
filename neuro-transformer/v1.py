...
# Domain classifier for advanced rewards
class DomainClassifier(nn.Module):
    """Simple classifier for domain-specific rewards"""
...
# Initialize domain classifier (science, humanities, code, etc.)
domain_classifier = DomainClassifier(model.config.hidden_size, 5).to(device)
domain_optimizer = AdamW(domain_classifier.parameters(), lr=1e-4)

# Domain labels for reward calculation
DOMAIN_LABELS = {
    "science": 0,
    "humanities": 1,
    "code": 2,
...
def train_step(batch, curriculum_stage, lambda_reward):
    ...
    with autocast():
        ...
        for i in range(len(gen_texts)):
            ...
            # 4. Reference-based Loss (BLEU + METEOR)
            bleu_score = calculate_bleu(responses[i], gen_response)
            meteor_score = calculate_meteor(responses[i], gen_response)
            ref_loss += 1.0 - (0.5 * bleu_score + 0.5 * meteor_score)
...
def validate(val_loader, curriculum_stage):
    ...
    with torch.no_grad():
        for instructions, responses in val_loader:
            # Similar to train_step but without weight updates
            max_length = next(s for s in CURRICULUM if s["name"] == curriculum_stage)["max_length"]
            inputs = tokenizer(
                instructions, 
                padding=True, 
